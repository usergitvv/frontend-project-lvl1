...
const cycleIterations = 4;

function generateGameProcess(workingFunction) {
  ...
  if (workingFunction[0] === 'What is the result of the expression?') {
    ...
    for (let i = 1; i <= cycleIterations; i += 1) {
      if (i === 4) {
  ...
  if (workingFunction[0] === 'Answer "yes" if the number is even, otherwise answer "no".') {
    for (let i = 1; i <= cycleIterations; i += 1) {
      ...
      console.log(`Question: ${workingFunction[i]}`);
      const userAnswer = readlineSync.question('Your answer: ');
      if (workingFunction[i] % 2 === 0 && userAnswer === 'yes') {
        console.log('Correct!');
      }
      if (workingFunction[i] % 2 !== 0 && userAnswer === 'no') {
        console.log('Correct!');
      }
      if (workingFunction[i] % 2 === 0 && userAnswer === 'no') {
  ...
  if (workingFunction[0] === 'What number is missing in the progression?') {
    for (let i = 1; i <= cycleIterations; i += 1) {
      ...
      if (workingFunction[2][i - 1] !== userAnswerNum) {
        ...
        \b\b\b\b\b\bis wrong answer ;(. Correct answer was '${workingFunction[2][i - 1]}'.\
        \nLet's try again, ${name}!`);
        break;
      }
    }
